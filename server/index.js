...
const start = async () => {
  ...
  app.post("/fetch", async (req, res) => {
    ...
    for (let i=0; i < req.body.emote.length; i++) {
      ...
      if (emote_i === 'All Chat Messages') {
        ...
        WHERE ts in '${date_i}' SAMPLE BY 2m)`;
      } else {
        q = `(SELECT ts, count() c FROM 'chatters' 
        WHERE message~'(?i)^.*${encodeURIComponent(emote_i)}.*$' 
        AND ts IN '${date_i}' SAMPLE BY 2m)`;
      }
      query_res = await c.query(q);
      var colors = palette('mpn65', req.body.emote.length);
      eresp.push({label: emote_i, type:'line', data:[], cubicInterpolationMode: 'monotone', backgroundColor: '#'+colors[i], borderColor: '#'+colors[i]});
  ...
  app.post("/topEmotes", async (req, res) => {
    const c = await pool.connect();
    console.log(req);
    q = `SELECT DISTINCT message, count() c FROM 'chatters' WHERE ts IN '${encodeURIComponent(req.body.date)}' ORDER BY c DESC LIMIT 100;`
    console.log(q);
